...
def train_model(model, train_loader, val_loader, device, optimizer, epochs=10, lr=1e-3):
    ...
    start_time = time.time()
    criterion = nn.CrossEntropyLoss(weight=torch.tensor([0.1, 1.0]))
...
'''if __name__ == "__main__":
    ...
    train_loader = DataLoader(train_dataset, batch_size=4, shuffle=True, num_workers=21, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=4, shuffle=False, num_workers=21, pin_memory=True)
    ...
    if training:
        ...
        train_losses, val_losses = train_model(model, train_loader, val_loader, device, optimizer, epochs=20, lr=1e-3)
        torch.save(model.state_dict(), f"{model_save_path}_training.pth")
        print(f"Model saved to {model_save_path}_training.pth")
