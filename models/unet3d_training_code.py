...
def save_model_on_interrupt(signal_received, frame, model, optimizer, model_save_path, start_epoch):
    print("\nInterrupt received! Saving model...")
    checkpoint = {
        'epoch': start_epoch,
        'model_state_dict': model.state_dict(),
...
def train_model(model, train_loader, val_loader, device, optimizer, epochs=10, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, masks) in enumerate(tqdm(train_loader, desc=f"Training Epoch {epoch + 1}")):
            ...
            print(f"Batch {batch_idx + 1}/{len(train_loader)} processed in {batch_time:.2f} seconds. Loss: {loss.item():.4f}")

            # Periodic GPU usage logging
            if (batch_idx + 1) % 10 == 0:
                log_gpu_usage()
...
if __name__ == "__main__":
    ...
    dataset_dir = f"{script_dir}/data/segmentai_dataset/processed/{region}_processed_dataset.pth"
    model_save_path = f"{script_dir}/models/unet/{region}_3d_unet_model"
    ...
    start_epoch = 0
    signal.signal(signal.SIGINT, lambda s, f: save_model_on_interrupt(s, f, model, optimizer, model_save_path, start_epoch))
    signal.signal(signal.SIGTERM, lambda s, f: save_model_on_interrupt(s, f, model, optimizer, model_save_path, start_epoch))
